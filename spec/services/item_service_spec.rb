  ...
  context '#create_item(data, user_id)'
    it 'creates an item for the given user' do
      ...
      item_data = {"name"=>"Vase", 
        ...
        item = ItemService.create_item(item_data, user[:data][:id])

        UserService.delete_user(user[:data][:id])
    end
  context '#get_items(id)' do 
    it 'gets all the items associated with a given user ID' do 
      ...
      item1 = {"name"=>"Vase", 
                ...
                "style"=>"Wheel-Thrown",
                "user_id"=> user[:data][:id]}
      ItemService.create_item(item1, user[:data][:id])
      ...
      response = ItemService.get_user_items(user[:data][:id], "none")
